...
import logging
import pandas as pd
from tag_keys import INFO_KEYS, DATE_PATTERN, MONTH, SKIP_KEYS,PATH,TARGET_KEYS
...
from pdfminer.layout import LAParams,  LTCurve, LTTextLine, LTTextBox,LTPage
...
from main_decorators import timeout, check_not_first_page_with_dict, controller,check_first_page_with_dict
...
def find_key(*args, **kwargs):
    ...
    if isinstance(element, LTTextBox):
        ...
        done = check_dict(item=current, the_dict=kwargs['the_dict'], *args, start=kwargs['start'])
        for do in done:
            if do[2]:
                return do, element
    return
...
def find_right_neightbors(media_bbox, layout, obj, width=3, pts=0.2, position='bbox'):
    ...
    if position == 'bbox':
        ...
        # y1 in the same line, and on the right position item.bbox[0] > obj.bbox[0]
...
class PageExtractor:
    ...
    def solve_points(self):
        # assign point and text box to object containers
    ...
    def quick_search(self):
        # quick search text boxes only ,
        pipeline = {}
        for keyword in TARGET_KEYS:
            pipeline[keyword] = []

        for textbox in self.text_box:
            logging.info("Quick searching textbox %s", textbox)
            slot = textbox.get_text().lower()
            for keyword in TARGET_KEYS:
                if keyword in slot:
                    # pipeline[keyword].append(textbox)
                    if keyword == keyword_modify(slot):
                        # this text box only contains keywords information need to search from
                        # neighbors.
                        pipeline[keyword].append((0,textbox))

                    else:
                        pipeline[keyword].append((1,textbox))             
...
class PDFExtractorFitz:
    ...
    def __iter__(self):
        for i in range(len(self.doc)):
            yield PageExtractorFitz( page = self.doc[i]) 
...
class PDFExtractor:
    ...
    """
    @check_first_page_with_dict(self.PAGE_DICT)
    class FirstPageInfo:
        """
                    ...
                    if isinstance(element, LTTextBox):
                        if element.bbox[2] - element.bbox[0] > 3:
                            ...
                            for k, v in DATE_PATTERN.items():
                                print("checking element date", k, v, element.get_text())
        ...
        def get_report_issue_date(self, pattern=None, guess=True):
            ...
            if self.potential_date_list:
                if pattern:
                    ...
                    report_issue_date = datetime.datetime(month=issue_date[0], day=issue_date[1], year=issue_date[2])
        ...
        def refine_lab(self):
            pattern = r"\((.*?)\)"
            if len(self.attr['lab']) > 25: # forget why greater than 25, but it works
    ...
    def __init__(self, fp, target=None):
        ...
        for pageid, page in enumerate(PDFPage.get_pages(self.fp), start=1):
            self.PAGE_DICT[pageid] = page.pageid
            self.DICT_PAGE[page.pageid] = pageid
    ...
    def collect_first_page_info(self):
        ...
        self.general_info = {'report_number': None, 'protocol': None, 'vendor_name': None, 'factory_name': None,
                             'lab': None}
    ...
    def get_begin_page(self, check_list):
        ...
        @check_not_first_page_with_dict(self.PAGE_DICT)
        @timeout(5)
...
if __name__ == '__main__':
    ...
    logging.basicConfig(format=format1, level=logging.WARN,
                        datefmt="%H:%M:%S")
    # logging.getLogger().setLevel(logging.DEBUG)
    path = PATH['path']
    sub_path = PATH['sub_path']['main']
    #for i in range(2):

    doc = Document(path=path, sub_path=sub_path, doc_id=1)
    doc()
    ...
    for page in sb:
        ws = wb1.create_sheet()
        page()
        page.page_to_excel(ws=ws, ref=DICT_PAGE)
        page.get_body(idx=-2)
    wb1.save('output/' + tag_name + '.xlsx')